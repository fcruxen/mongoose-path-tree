 ...
 *
 * Implements the materialized path strategy with cascade child re-ancestrying
...
function tree(schema, options) {
    ...
    /**
     * Add ancestry and path properties
     *
     * @property {ObjectID} ancestry
    ...
    schema.pre('save', function preSave(next) {
        var isParentChange = this.isModified('ancestry');
        ...
        if (this.isNew || isParentChange) {
            ...
            var self = this;
            this.collection.findOne({ _id: this.ancestry }, function (err, doc) {
                ...
                if (isParentChange) {
                    // When the ancestry is changed we must rewrite all children paths as well
    ...
    schema.pre('remove', function preRemove(next) {
        ...
        if (onDelete == 'DELETE') {
            ...
            var self = this,
                newParent = this.ancestry,
                previousParent = this._id;

            // Update ancestry property from children
            this.collection.find({ ancestry: previousParent }, function (err, cursor) {
                ...
                streamWorker(cursor.stream(), numWorkers, function streamOnData(doc, done) {
                        ...
                        self.collection.update({ _id: doc._id }, { $set: { ancestry: newParent } }, done);
                },
    ...
    schema.methods.getChildren = function getChildren(filters, fields, options, recursive, next) {
        ...
        if (recursive) {
            ...
            if(filters['$query']){
                filters['$query']['ancestry'] = this._id;
            } else {
                filters['ancestry'] = this._id;
            }
        }
    ...
    schema.methods.getParent = function getParent(next) {
        ...
        return this.model(this.constructor.modelName).findOne({ _id: this.ancestry }, next);
    };
    ...
    schema.statics.getChildrenTree = function getChildrenTree(root, args, next) {
        ...
        if (recursive) {
            ...
            if (filters.ancestry === null) {
                delete filters.ancestry;
            }

        } else {
            if (root) {
                filters.ancestry = root._id;
            }
            else {
        ...
        // fields: Add path and ancestry in the result if not already specified
        if (fields) {
